...
KdTreeNode* KdTree::InsertNode(KdTreeNode* parent, KdTreeNode* node, int key[],
                               double value) {
  if (node == NULL) {
    ...
    for (int i = 0; i < 3; ++i) node->key[i] = key[i];
...
void KdTree::ClusterNode(KdTreeNode* node, int depth) {
  ...
  for (int i = 0; i < 3 * 3 * 3; ++i) {
    ...
    // This node already has a leabel; skip it. The label should be consistent,
    // however
    if (nnode->cluster >= 0) {
